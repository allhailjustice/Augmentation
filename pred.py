...
if __name__ == '__main__':
    ...
    lengths, features, targets, histories, summaries = np.load('../XINMENG/length.npy').astype('int32'), \
                                                       np.load('../XINMENG/code.npy').astype('int32') + 1, \
                                                       np.load('../XINMENG/target.npy').astype('int32') + 1, \
                                                       np.load('../XINMENG/history.npy').astype('int32') + 1, \
                                                       np.load('../XINMENG/others.npy').astype('float32')

    train_idx, test_idx, val_idx = np.load('train_idx_'+str(p)+'.npy'), np.load('../XINMENG/test_idx.npy'), np.load('val_idx_'+str(p)+'.npy')
    ...
    for i in range(20):
        auc.append(train())
    np.save('result/total_'+str(p), auc)
